...
#include <tesseract_examples/ureca_penetration_failure_3d_example.hpp>
...
Eigen::VectorXd makeSeedPoint(int i, int k, int n, const Eigen::VectorXd& start, const Eigen::VectorXd& subgoal, const Eigen::VectorXd& goal)
{
  if (i <= k)
...
CompositeInstruction buildProgram(const std::vector<std::string>& joint_names,
                                  ...
                                  int k)
{
  CompositeInstruction program("ureca_penetration_failure_3d_program",
...
std::shared_ptr<tesseract_common::ProfileDictionary> createProfiles(double safety_margin)
{
  auto profiles = std::make_shared<tesseract_common::ProfileDictionary>();
  auto composite_profile = std::make_shared<TrajOptDefaultCompositeProfile>();
  composite_profile->collision_cost_config = trajopt_common::TrajOptCollisionConfig(safety_margin, 1000);
  ...
  composite_profile->collision_constraint_config = trajopt_common::TrajOptCollisionConfig(safety_margin, 20);
  ...
  profiles->addProfile(TRAJOPT_NAMESPACE, "ureca_penetration_failure_3d_program", composite_profile);
...
UrecaPenetrationFailure3DExample::UrecaPenetrationFailure3DExample(
...
bool UrecaPenetrationFailure3DExample::run()
{
  ...
  const std::filesystem::path result_dir("/home/wenda/tesseract_ws/result_ureca/penetration_failure_3d");
  urecaEnsureResultLayout(result_dir);
  TrajOptMotionPlanner planner(TRAJOPT_NAMESPACE);

  const std::vector<PenetrationCase> cases{
    { "shallow_penetration", 0.00 },
    { "medium_penetration", 0.05 },
    { "deep_penetration", 0.10 },
    { "very_deep_penetration", 0.20 },
  };

  std::vector<UrecaSummaryRow> rows;
  for (const auto& penetration_case : cases)
  {
    CompositeInstruction program = buildProgram(joint_names, start, subgoal, goal, n, k);
    auto profiles = createProfiles(penetration_case.safety_margin);
    ...
    UrecaSummaryRow row;
    row.case_id = penetration_case.label;
    row.K = k;
    row.penetration_label = penetration_case.label;
    ...
    if (response.successful)
    {
      ...
      if (extractTrajectory(response.results, opt_traj))
      {
        const double min_clearance = urecaComputeMinSphereObstacleClearanceProxy(
            opt_traj, OBSTACLE_CENTER, OBSTACLE_RADIUS + penetration_case.safety_margin, ROBOT_RADIUS, 0, 3);
