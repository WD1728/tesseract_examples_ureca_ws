...
bool BasicCartesianExample::run()
{
  ...
  // -----------------------------
  // Inspect Output Trajectory
  // -----------------------------
  if (!future->context->isSuccessful())
  {
    CONSOLE_BRIDGE_logError("TrajOptPipeline failed.");
    return false;
  }
  ...
  std::cout << "trajectory size = " << trajectory.states.size() << std::endl;
  ...
  for (std::size_t i = 0; i < trajectory.states.size(); ++i)
  {
    ...
    for (std::size_t j = 0; j < state.joint_names.size(); ++j)
    {
      std::cout << state.joint_names[j] << "=" << state.position[j] << " ";
    }
